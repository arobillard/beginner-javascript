...
const ourNames = Array.of('adam', 'hannah');
...
const filterMinRating = (feedbackList, minRating) =>
  feedbackList.filter(singleFeedback => singleFeedback.rating >= minRating);

// console.table(goodReviews);
// console.table(filterMinRating(feedback, 2));
...
// Remove the one star rating however you like!
const legitRatings = feedback.filter(poor => poor.rating !== 1);
// console.table(legitRatings);

// check if there is at least 5 of one type of meat with some()
const isThereMeat = Object.values(meats).some(meatValue => meatValue >= 5);
// console.log(isThereMeat);
// make sure we have at least 3 of every meat with every()
const isThereEnoughEveryMeat = Object.values(meats).every(meatValue => meatValue >= 6);
// console.log(isThereEnoughEveryMeat);

// sort the toppings alphabetically with sort()
const numbers = [1, 2, 100, 3, 200, 500, 155];
